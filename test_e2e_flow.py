...
def run_test():
    ...
    # 1. Question Bank
    bank_name = "Test DSA Question Bank"
    if not frappe.db.exists("Coding Question Bank", bank_name):
        bank = frappe.get_doc({
        ...
        bank = frappe.get_doc("Coding Question Bank", bank_name)
        print("[INFO] 1. Using existing Question Bank:", bank.name)

    # 2. Coding Problem
    prob_title = "Sum of First N Numbers"
    existing = frappe.db.get_value("Coding Problem", {"title": prob_title}, "name")
    if existing:
        frappe.delete_doc("Coding Problem", existing, force=1)
